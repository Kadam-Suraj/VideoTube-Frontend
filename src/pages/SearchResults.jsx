...
const SearchResults = ({ className }) => {
    ...
    return (
        <div className="grid mx-auto max-w-screen-lg min-h-80">
            {
                isLoading ?
                    <Loading /> :
                    data?.videos?.length || data?.users?.length > 0 ?
                        <div className="space-y-5 border-separate">
                            {
                                data?.users?.length > 0 &&
                                        ...
                                        data.users.map((item) => {
                                            return (
                                                <div key={item?._id} className="flex items-center space-x-3">
                                                    <NavLink to={`/user/@${item?.username}`} className="flex items-center space-x-3 w-full">
                                                    ...
                                                    <Subscribe fnc={fetchData} owner={item} className="justify-self-end text-end" />
                                                </div>
                                            )
                                        })
                                    }
