...
const PlaylistVideos = ({ className }) => {
    ...
    return (
        <div className={`flex flex-col space-y-3 max-h-[25rem] overflow-y-scroll w-full rounded-md border ${className}`}>
            <div className="flex flex-col gap-2 p-2 rounded-t-md bg-muted">
                <h2 className="text-2xl font-semibold">{playlist?.name || "Untitled"}</h2>
                <div className="flex flex-wrap items-center gap-2">
                    {
                        !playlist?.isPublic &&
                        <span className="flex items-center gap-2 px-2 rounded-md text-muted-foreground bg-accent-foreground/10">
                            <LockKeyhole size={16} />
                            <span>
                                Private
                            </span>
                    ...
                    playlist?.videos && playlist?.videos?.length > 0 ?
                        <div className="grid p-2 space-y-2">
                            {playlist?.videos.map((item, key) => {
                                ...
                                return <NavLink key={key} to={`/watch?v=${item._id}&playlist=${playlistId}`} className="w-full shrink">
                                    <Card className="mx-auto transition-all duration-300 shrink sm:hover:scale-[1.03] sm:hover:shadow-lg p-0 border-none">
                                        <div className="flex items-center">
                                            <div className={`flex w-full relative gap-1 justify-between ${item?._id === videoId && "bg-accent border border-border"} hover:bg-accent rounded-md cursor-pointer min-h-28 max-[363px]:flex-col md:flex-col xl:flex-row`}>
